...
<template>
    ...
    <rect :x="0 + 450" :y="2" :width="9" :height="9" :fill="'#181CF7'" :stroke="'#534f4f'"></rect>
        ...
        <marker
          ...
          <path d="M 0 0 L 10 5 L 0 10 z" stroke-width="100" />
        </marker>
      </defs>
                                                                        <path v-for="(p_item, p_i) in projectFlow" :key="'flow' + p_i" :d="p_item.d" :fill="'none'" :stroke="p_item.dir == 1 ? '#181CF7' : '#A50021'" :stroke-width="p_item.w" opacity="0.5" ></path>
                                                                    </g>
                                                                    <g v-show="showHSB">
                                                                        <path
        ...
        calcScatter(data, powIndex) {
            ...
            if (powIndex == 2) {
                ...
                for (let i in scatterData) {
                    center.x += scatterData[i].x;
                    center.y += scatterData[i].y;
                }
                ...
                let maxLen = 0;
                for (let i in scatterData) {
                    let len = Math.sqrt((scatterData[i].x - center.x) * (scatterData[i].x - center.x) + (scatterData[i].y - center.y) * (scatterData[i].y - center.y));
                    maxLen = Math.max(maxLen, len);
                }
                let lenScale = scaleLinear([0, maxLen], [0, 1200]);
                for (let i in scatterData) {
                    let len = Math.sqrt((scatterData[i].x - center.x) * (scatterData[i].x - center.x) + (scatterData[i].y - center.y) * (scatterData[i].y - center.y));
                    if (len != 0) {
                        scatterData[i].ax = (scatterData[i].x - center.x) * Math.pow(len * 10000, 1 / 100) * 1000;
                        scatterData[i].ay = (scatterData[i].y - center.y) * Math.pow(len * 10000, 1 / 100) * 1000;
                    } else {
                        scatterData[i].ax = (scatterData[i].x - center.x);
                        scatterData[i].ay = (scatterData[i].y - center.y);
                    }
