        ...
        <div class="frameworkBody" style="padding-bottom: 5px;">
            ...
            <div style="height: 100%; width: 70%; float: right; overflow-x: scroll; overflow-y: hidden; white-space: nowrap" ref="groupView">
                ...
                <svg height="100%" width="100%">
                    <g :transform="translate(70, 30, 0)">
                        <text :x="0" y="0" text-anchor="middle">M1</text>
                            <text :x="0" y="20" text-anchor="middle">1</text>
                            <text :x="0" :y="groupHeight - 40" text-anchor="middle">0</text>
                        <path :d="'M 0 30 L 0 ' + (groupHeight - 60)" fill="none" stroke="black"></path>
                    </g>
                    <g :transform="translate((groupWidth - 140) * 1 / 3 + 70, 30, 0)">
                            <text :x="0" y="0" text-anchor="middle">M2</text>
                                <text :x="0" y="20" text-anchor="middle">1</text>
                                <text :x="0" :y="groupHeight - 40" text-anchor="middle">0</text>
                            <path :d="'M 0 30 L 0 ' + (groupHeight - 60)" fill="none" stroke="black"></path>
                    </g>
                    <g :transform="translate((groupWidth - 140) * 2 / 3 + 70, 30, 0)">
                            <text :x="0" y="0" text-anchor="middle">M3</text>
                                <text :x="0" y="20" text-anchor="middle">1</text>
                                <text :x="0" :y="groupHeight - 40" text-anchor="middle">0</text>
                            <path :d="'M 0 30 L 0 ' + (groupHeight - 60)" fill="none" stroke="black"></path>
                    </g>
                    
                    <g :transform="translate((groupWidth - 140) * 3 / 3 + 70, 30, 0)">
                            <text :x="0" y="0" text-anchor="middle">IMP</text>
                                <text :x="0" y="20" text-anchor="middle">1</text>
                                <text :x="0" :y="groupHeight - 40" text-anchor="middle">0</text>
                            <path :d="'M 0 30 L 0 ' + (groupHeight - 60)" fill="none" stroke="black"></path>
                    </g>
                    <g :transform="translate(0, 30, 0)">
                        <path v-for="(o, i) in parallelData" :key="'parallel' + i" :d="o.d" :fill="'none'" :stroke="o.stroke"></path>
                    </g>
                </svg>
                <svg v-show="0" v-for="(item, i) in groupSet" :key="'group' + i" height="100%" width="25%">
...
import alternative_data from '../assets/NFT_data/8/control_panel.json';
...
export default {
    ...
    methods: {
        ...
        calcParallel (data) {
            let m1Min = 999999, m1Max = -999999;
            let m2Min = 999999, m2Max = -999999;
            let m3Min = 999999, m3Max = -999999;
            let impMin = 999999, impMax = -999999;
            for (let i in data) {
                // console.log(data[i]);
                m1Min = Math.min(m1Min, data[i]['M1']);
                impMin = Math.min(impMin, data[i]['IMP']);
                m2Min = Math.min(m2Min, data[i]['M2']); 
                m3Min = Math.min(m3Min, data[i]['M3']);
                m1Max = Math.max(m1Max, data[i]['M1']);
                impMax = Math.max(impMax, data[i]['IMP']);
                m2Max = Math.max(m2Max, data[i]['M2']);
                m3Max = Math.max(m3Max, data[i]['M3']);
            }
            let m1Scale = scaleLinear([m1Min, m1Max], [0, 1]);
            let m2Scale = scaleLinear([m2Min, m2Max], [0, 1]);
            let m3Scale = scaleLinear([m3Min, m3Max], [0, 1]);
            let impScale = scaleLinear([impMin, impMax], [0, 1]);
            let yScale = scaleLinear([0, 1], [this.groupHeight - 60, 30]);
            let res_data = [];
            for (let i in data) {
                // if (data[i]['Group'] !=) continue;
                // console.log(data[i]['Group']);
                let d_p = 'M ' + (70) + ' ' + (yScale(m1Scale(data[i]['M1']))) + ' L ' + (70 + (this.groupWidth - 140) / 3) + ' ' + (yScale(m2Scale(data[i]['M2']))) + ' L ' + (70 + (this.groupWidth - 140) * 2 / 3) + ' ' + (yScale(m3Scale(data[i]['M3']))) + ' L ' + (70 + (this.groupWidth - 140)) + ' ' + (yScale(impScale(data[i]['IMP'])));
                res_data.push({
                    d: d_p,
                    group: data[i]['Group'],
                    stroke: this.colormap[data[i]['Group']]
                })
            }
    ...
    created() {},
    mounted() {
        // console.log(alternative_data.data);
        ...
        this.parallelData = this.calcParallel(alternative_data.data);
