                    ...
                    <svg width="100%" height="100%">
                                                            ...
                                                            <g>
                                                                <g v-for="(item, i) in correlationData" :key="'correlation_circle_' + i"
                                                                    ...
                                                                    <circle :x="0" :y="0" :fill="'white'" :stroke="item.circleData.stroke" :r="item.circleData.r" :opacity="item.opacity" :stroke-width="2"></circle>
                                                                
                                                                    <path v-for="(a_item, a_i) in item.outArc" :key="'corr_out_' + a_i" :d="a_item.d"
                                                                        :fill="a_item.fill"></path>
                                                                        <circle :x="0" :y="0" :r="item.circleData.r - 8" :fill="'none'" :stroke="'white'"></circle>
                    ...
                    <svg width="100%" height="100%">
                            ...
                            <g>
                                                                        <g v-for="(item, i) in correlationData" :key="'selectKey' + i">
                                                                            <path v-for="(p_item, p_i) in item.lineData" :key="'select_line_' + p_i"  :transform="translate(0, 25, 0)" :d="'M' + (p_item.x + p_item.rw) + ',' + p_item.y + 'L' + (p_item.x + p_item.rw) + ',' + (p_item.y + p_item.h)" :fill="'none'" :stroke="p_item.stroke" :stroke-width="3" :opacity="item.opacity"></path>
                                                                        </g>
                                                                    </g>
...
export default {
    ...
    methods: {
        ...
        calcIndividualProject(data1, data2, data3) {
            let max_m1 = 0,
                max_m2 = 0,
                max_m3 = 0,
                max_imp = 0;
            ...
            for (let i in data3) {
                ...
                max_m3 = Math.max(max_m3, data3[i].M3)
                max_imp = Math.max(max_imp, data3[i].IMP);
                min_m1 = Math.min(min_m1, data3[i].M1);
                min_m2 = Math.min(min_m2, data3[i].M2);
                min_m3 = Math.min(min_m3, data3[i].M3);
                min_imp = Math.min(min_imp, data3[i].IMP);
            }
            let data = [];
            for (let i in data1) {
                let t_data = {};
